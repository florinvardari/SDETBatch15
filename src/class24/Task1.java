...
Create objects of child classes and store them into array. Loop through each object and execute available methods.
    ...
    for (Computer c:computers){
        ...
        if (c instanceof Apple){// we are checking if the variable c contains the object of an Apple class
            Apple apple=(Apple)c;// we are converting a variable c back to an apple object
            apple.installApp();
            apple.editVideos();
        }
...
class Apple extends Computer{
    void playVideo(){
        System.out.println("Playing the video on Quick TimePlayer");
    }
    ...
    void editVideos(){
        System.out.println("Editing on IMovie");
    }
    void installApp(){
